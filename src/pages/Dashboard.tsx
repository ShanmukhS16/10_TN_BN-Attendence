...
/**
 * Dashboard
 * - Mobile: partial-width drawer (~75%) + dim overlay
 * - Desktop: collapsible sidebar (icon rail when collapsed)
 * - Single Reports entry (sidebar)
 * - Overview refined (no Reports button)
 * - Admin sections: Manage Users, Upload Students
 */
...
const Dashboard: React.FC = () => {
  ...
  const promoteToAdmin = async () => {
    ...
    const { error } = await supabase.from("users").update({ role: "admin" }).eq("id", selectedUserId);
    if (error) {
      setMessage("❌ Failed to promote: " + error.message);
    } else {
      setMessage("✅ User promoted to admin successfully!");
      setAllUsers((prev) => prev.map((u) => (u.id === selectedUserId ? { ...u, role: "admin" } : u)));
      setSelectedUserId("");
    }
  };
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      const rowsToInsert: any[] = studentsPreview.map((r: any) => {
        const name = pickString(r, ["name", "Name", "studentName", "student_name"]);
        const regimentalNo = pickString(r, ["regimentalNo", "regimental_no", "regimentalno", "Regimental No", "Regimental"]);
        const rank = pickString(r, ["rank", "Rank"]);
        const collegeRaw = pickString(r, ["collegeId", "college_id", "college", "College"]);
        const total_classes = pickNumber(r, ["total_classes", "totalClasses", "total classes"]);
        const attended_classes = pickNumber(r, ["attended_classes", "attendedClasses", "attended classes"]);
        return { name, regimentalNo, rank, collegeRaw, total_classes, attended_classes, createdBy: user.id };
      });

      // map collegeRaw to id
      ...
      const existingQuery = regNos.length
        ? await supabase.from("students").select("regimentalNo").in("regimentalNo", regNos)
        : { data: [], error: null };
      const existingSet = new Set((existingQuery.data || []).map((x: any) => String(x.regimentalNo)));
      ...
      for (const row of rowsToInsert) {
        ...
        if (row.collegeRaw) {
          ...
          if (!collegeId) {
            const found = (colleges || []).find(
              (c: any) =>
                (c.name && c.name.toLowerCase().includes(row.collegeRaw.toLowerCase())) ||
                (c.code && c.code.toLowerCase().includes(row.collegeRaw.toLowerCase()))
            );
        ...
        finalInsert.push({
          ...
          attendancePercentage:
            row.total_classes && row.total_classes > 0 ? Math.round((row.attended_classes / row.total_classes) * 100) : 0,
      ...
      if (!finalInsert.length) {
        ...
        if (skipped.length) msg += ` Skipped ${skipped.length} rows (missing/duplicates).`;
        alert(msg);
        setUploading(false);
        return;
      }
      ...
      await fetchData?.();
      alert(`✅ Inserted ${finalInsert.length} students. Skipped ${skipped.length} rows.`);
      setStudentsPreview([]);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-100 via-indigo-100 to-purple-100">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-white/70 border-b border-gray-200 shadow-sm">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-gray-900">10 TN BN Dashboard</h1>
              <p className="text-xs text-gray-600">
                Welcome, {user.name} • {user.email} • <span className="uppercase">{user.role}</span>
              </p>
            </div>
          </div>
      ...
      {/* Sidebar: partial-width drawer on mobile, collapsible on desktop */}
      <aside
        className={[
          "fixed inset-y-0 left-0 z-50 bg-white/90 backdrop-blur-md border-r border-gray-200 shadow-xl",
          // Mobile width: partial (~75%) with max
          "w-3/4 max-w-[18rem] md:max-w-none",
          // Translate behavior (mobile)
          "transform transition-transform duration-300 md:translate-x-0",
          sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0",
          // Desktop widths (collapsed vs expanded)
          sidebarCollapsed ? "md:w-20" : "md:w-72",
          // Make it relative on desktop layout
          "md:fixed",
        ].join(" ")}
      >
        {/* Sidebar header */}
      ...
      {/* Content (pad by sidebar width on desktop) */}
        ...
        <div className="container mx-auto px-6 py-8">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-600">
              {section === "overview" && "Organization snapshot and quick actions."}
              {section === "users" && "Promote, remove, and view all users."}
              {section === "upload" && "Bulk import students from Excel/CSV."}
            </p>
          </div>

          {/* OVERVIEW — refined (no Reports card) */}
          {section === "overview" && (
                  ...
                  <div className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl shadow-md p-5 hover:shadow-lg transition">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">Promote admins, remove users</p>
                      </div>
                      <div className="w-10 h-10 rounded-lg bg-emerald-100 flex items-center justify-center">
                        <Users className="w-5 h-5 text-emerald-600" />
                      </div>
                    </div>
                    <Button onClick={() => setSection("users")} variant="outline" className="mt-4 w-full">
                  ...
                  <div className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl shadow-md p-5 hover:shadow-lg transition">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">Bulk add via Excel/CSV</p>
                      </div>
                      <div className="w-10 h-10 rounded-lg bg-amber-100 flex items-center justify-center">
                        <UploadCloud className="w-5 h-5 text-amber-600" />
                      </div>
                    </div>
                    <Button onClick={() => setSection("upload")} variant="outline" className="mt-4 w-full">
                    ...
                    <tbody>
                      {allUsers.map((u) => (
                        <tr key={u.id} className="bg-white/60 hover:bg-gray-50">
                          ...
                          <td className="p-2 border-b">{u.approved ? "✅" : "⛔"}</td>
          ...
          {section === "upload" && isAdmin && (
            <div className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl shadow-md p-6 space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Upload .xlsx / .xls / .csv with columns like: <code>name</code>,{" "}
                  <code>regimentalNo</code>, <code>rank</code>, <code>college</code>,{" "}
                  <code>total_classes</code>, <code>attended_classes</code>.
                </p>
              </div>
                      ...
                      <thead className="bg-gray-100">
                        <tr>
                          {Object.keys(studentsPreview[0] || {}).map((k) => (
                            <th key={k} className="px-2 py-1 border-b text-left">{k}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {studentsPreview.slice(0, 10).map((row, idx) => (
                          <tr key={idx} className="odd:bg-white/70 even:bg-gray-50/60">
                            {Object.keys(studentsPreview[0] || {}).map((k) => (
                              <td key={k} className="px-2 py-1 border-b">{String(row[k] ?? "")}</td>
                            ))}
                          </tr>
                        ))}
                      </tbody>
